...
## 20. 言語を渡さないと日本語の歌詞は歌われない — 2026-09-14

音楽 GUI は動いたが、日本語だと歌が短い・おかしいという報告。同じ歌詞・同じ prompt で
30 秒を 2 本作って聴き比べたところ、**`vocal_language: "ja"` を指定したほうだけが歌った**。
指定しないほうは伴奏だけだった。

原因は経路にあった。`frontend/app.js` は `vocal_language` を**詳細モードのセレクトから
しか設定しない**。簡易モードで日本語の歌詞を書いても言語は宣言されず、ACE-Step は
`unknown` を受け取る。画面の簡易モードから頼むと必ずこれに当たっていた。

直したもの: `backend/sonicforge/vocal_language.py` を足し、歌詞の文字から言語を決める。
かな→ja、ハングル→ko、仮名の無い漢字→zh、キリル文字→ru。**ラテン文字は英語・
スペイン語などを見分けられないので決めない**——確実に分かるときだけ名乗る。判定は
要求の検証に置いたので、画面からも MCP からも同じに効く。人が選んだ値は上書きしない。

自前の計測（ピッチの続く割合）では有意な差が出なかった（35.0% 対 31.6%）。チップチューンの
旋律と歌声をピッチだけでは区別できない。**ここは耳が判定の道具である。**

「ボイスが短い」のもう一方の要因は歌詞の量と思われる。ACE-Step は渡した歌詞を一度
歌って終わるので、4 行の歌詞を 120 秒に渡せば残りは伴奏になる。契約の説明へ書いた。

## 21. 説明文を実態へ合わせる — 2026-09-14

MCP の道具一覧と契約に出る説明が、§13〜§20 の変更を反映していなかった。呼ぶ側が読むのは
この文面だけなので、実装だけ直しても届かない。

- `schemas/generate-request.json` / `generate-batch-request.json` の説明へ、歌詞・ループ・
  長さの範囲を追記。「instrumental を false にするだけでは歌わない」「重ねたぶん短くなる」
  「長さの範囲は task ごとに違う」を明示した。
- `schemas/speech-transcribe-request.json` の説明を日本語にし、音の在りかの 3 つの道と、
  自分で作った asset をそのまま渡せることを書いた。
- `addon.json` の label を実態へ。`sonic.generate` は「speech, a sound effect, ambience or
  music (with lyrics, loop)」、`sonic.transcribe` は「from an asset, an upload or a project
  file」。一覧に出るのはこの 1 行だけなので、何が頼めるかが読み取れる語にする。

確認: 全 221 件の pytest が成功。

## 22. LLM が使用中でも音楽を作れるようにする — 2026-09-14

音楽は 20 GiB を要求して device を占有する。broker は常駐 LLM へ退去を頼むが、
**退くかどうかを決めるのは LLM 側**で、使用中なら断る。結果、OpenCode が LLM を
使っている間、音楽は延々と待つ（実測: 画面に `Waiting for GPU:
device_busy_exclusive` が出たまま 9 分進まず）。

### 測ったこと

LLM が 22.9 GiB を常駐したまま（空き 11.3 GiB）30 秒の曲を作れるか:

| 構成 | 音楽が使う VRAM | 結果 |
|---|---|---|
| 既定（fp32・送り出し無し） | 約 21 GiB | 同居不可 |
| 送り出しのみ | 約 10 GiB で頭打ち | **OOM** |
| int8 のみ | 約 11.6 GiB で頭打ち | **OOM** |
| **int8 + 送り出し** | **8.47 GiB** | **成功**（132 秒） |

単独ではどちらも僅かに足りず、併用して初めて収まる。

DiT の内訳（ROCm では既定が float32 なので、保存 4.5 GB に対しメモリ上は 9.58 GB）:

```
decoder 線形 6.30 GB / encoder 線形 2.43 GB / tokenizer 0.42 / detokenizer 0.42
```

量子化の範囲と代償（同じ seed・同じ prompt・30 秒）:

| 範囲 | DiT | 生成時間 |
|---|---|---|
| 既定（量子化なし） | 9.58 GB | 147 秒 |
| decoder のみ（採用） | 4.86 GB | 147 秒 |
| 全部（tokenizer 含む） | 2.3 GB | **344 秒** |

全部に広げると 1/4 まで落ちるが 2.3 倍遅い。ACE-Step 自身の量子化も `decoder.*`
だけなので、そこに合わせた。

### 直したもの

- 音楽の資源要求へ `minimum_bytes`（10 GiB）を申告。これが無いと broker は
  「20 GiB 全部か、さもなくば待て」しか選べない。
- broker が実際に貸した枠（`granted_bytes`）を worker まで運ぶ。
- worker は枠を見て載せ方を決める。全部載る枠なら従来どおり最速。足りなければ
  DiT を int8 にして部品ごとに送り出す。結果に `placement` と
  `granted_vram_bytes` を残す。
- int8 は**無ければ作って file へ保存し、あれば読むだけ**（作成は CPU で 9 秒、
  読み込みは 2.0 秒）。作った直後も file から読み直す——初回だけ別の道を通ると、
  そこだけ壊れていても気づけない。GPU を使わずに先に作る入口
  （`prepare_int8`）も持たせた。
- 枠を絞って走っている間は worker を降ろさない。重みは RAM に置くので VRAM を
  握っておらず、2 本目からモデルの読み直しが要らない。

**途中で 1 つ間違えた。** 送り出しを使うときに VAE や text encoder まで手で GPU へ
移していて、ACE-Step 自身の出し入れと衝突して実機で 2 本とも
「mat2 is on cpu」で落ちた。送り出し時は DiT だけを移す（そうでないときは一式）。
試験を足した。

### 実機での確認

LLM が 22.8 GiB を掴んで処理中のまま、MCP から 30 秒の曲を続けて 2 本:

```
1本目  202.7 秒  placement=int8_offload  枠 11.27 GB
2本目   78.2 秒  placement=full_device   枠 22.01 GB  ← LLM が空いたので全部載せた
```

待たずに作れて、枠が空けば自動で速い側へ戻る。全 226 件の pytest が成功。

...
# 注意は説明に書く）。
# 効果音・環境音（Stable Audio）: 下限は 2 秒。worker は 0.1 秒から受けるが、
# **受け付けることと使える音が出ることは別**である。
#
# モデルは「指定 + 6 秒」を作ってから先頭の指定秒だけを切り出す
# （stable_audio_3 の duration_padding_sec / truncate_output_to_duration）。
# 2 秒を下回ると、切り出されるのは音が立ち上がる前の過渡部分だけになり、
# 飽和した矩形波が返る。実測 2026-09-14〜15、同じ prompt で 12 件:
#
#   0.10〜0.80 秒（9 件） 尖頭 1.000・波高率 1.14〜1.25 ＝ 飽和した雑音
#   1.00 秒              尖頭 0.001 ＝ ほぼ無音
#   1.50 秒              音が端まで詰まって切れている
#   2.00 秒以上          音は前半で終わり、後ろに無音が残る ＝ 使える
#
# 短い効果音は「短く頼む」のではなく、2 秒で作って後ろの無音を落として得る。
DURATION_LIMITS: dict[str, tuple[float, float]] = {
    "audio.sfx.generate": (2.0, 120.0),
    "audio.ambience.generate": (2.0, 120.0),

...
| Progressive LLM -> TTS | UNIT OVERLAP + REAL SEQUENTIAL FALLBACK PASS | bounded SSE/TTS/audio overlap remains available when admitted; the target 32 GiB route now fails overlap admission immediately, drains the LLM stream, releases Host AI and runs TTS sequentially instead of deadlocking |
